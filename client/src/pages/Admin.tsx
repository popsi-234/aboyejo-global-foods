...
export default function Admin() {
  ...
  const [pendingDelete, setPendingDelete] = useState<{ table: "products" | "product_categories" | "gallery" | "souvenir_packages" | "faqs" | "testimonials" | "newsletter_subscribers"; id: string; label: string } | null>(null);
  ...
  useEffect(() => {
    const handleDragOver = (event: DragEvent) => {
      ...
      if (target?.closest('label')?.querySelector('input[name="image_file"][type="file"]')) event.preventDefault();
    };
    const handleDrop = (event: DragEvent) => {
      const target = event.target instanceof Element ? event.target : null;
      const input = target?.closest('label')?.querySelector<HTMLInputElement>('input[name="image_file"][type="file"]');
      const files = event.dataTransfer?.files;
      if (!input || !files?.length) return;
      const accepted = ["image/jpeg", "image/png", "image/webp"];
      if (!accepted.includes(files[0].type)) { setNotice("Choose a JPEG, PNG, or WebP image up to the bucket limit."); return; }
      event.preventDefault();
      input.files = files;
      input.dispatchEvent(new Event("change", { bubbles: true }));
      setNotice(`${files[0].name} is ready to upload.`);
    };
  ...
  function requestDelete(table: "products" | "product_categories" | "gallery" | "souvenir_packages" | "faqs" | "testimonials" | "newsletter_subscribers", id: string, label: string) { setPendingDelete({ table, id, label }); setNotice(`Confirm deletion of this ${label}.`); }
  const deleteRecord = requestDelete;
  function storagePathFromPublicUrl(url: string, bucket: string) { const marker = `/storage/v1/object/public/${bucket}/`; const markerIndex = url.indexOf(marker); return markerIndex === -1 ? null : decodeURIComponent(url.slice(markerIndex + marker.length)); }
  async function confirmDelete() {
    if (!pendingDelete) return;
    const { table, id, label } = pendingDelete;
    const media = table === "products" ? products.find((item) => item.id === id) : table === "gallery" ? gallery.find((item) => item.id === id) : table === "souvenir_packages" ? souvenirs.find((item) => item.id === id) : null;
    const bucket = table === "products" ? "product-images" : table === "gallery" ? "gallery-images" : table === "souvenir_packages" ? "souvenir-images" : null;
    const imagePath = media?.image_url && bucket ? storagePathFromPublicUrl(media.image_url, bucket) : null;
    setPendingDelete(null);
    const { error } = await supabase.from(table).delete().eq("id", id);
    if (error) { setNotice(error.message); return; }
    if (imagePath && bucket) {
      const { error: storageError } = await supabase.storage.from(bucket).remove([imagePath]);
      if (storageError) { setNotice(`${label} deleted, but its Storage image could not be removed: ${storageError.message}`); void refresh(); return; }
    }

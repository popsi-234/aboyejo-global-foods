...
export function ContactPage() {
  ...
  return <PublicShell><PageHero eyebrow="Order enquiry" title={<>Ready when<br /><i>you are.</i></>}>Tell the team what you are planning. Use the form for an enquiry, or WhatsApp when the live line has been configured.</PageHero><section className="premium-content-section"><div className="premium-content-intro split"><p className="premium-eyebrow">04 / Start the conversation</p><p>Share the details of your pantry order, occasion, or gifting plan. The team will respond through the channel you choose.</p></div><div className="premium-contact-grid"><form className="premium-contact-form" onSubmit={onContact}><div className="premium-form-heading"><MessageCircle size={20} /><div><p className="premium-eyebrow">Direct enquiry</p><h2>Tell us what you need.</h2></div></div><div className="premium-form-split"><label>Your name<input name="name" required placeholder="Full name" /></label><label>Phone number<input name="phone" placeholder="e.g. +234…" /></label></div><label>Email <span>(optional)</span><input type="email" name="email" placeholder="you@example.com" /></label><label>What are you planning?<textarea name="message" required rows={6} placeholder="Pantry order, occasion, gifting, or something else…" /></label><button className="premium-form-button" type="submit"><Send size={16} /> Send enquiry</button></form><aside className="premium-contact-aside"><p className="premium-eyebrow">Ordering channel</p><h2>WhatsApp, when you need it.</h2><p>For quick follow-up on products and occasions, the business keeps its live WhatsApp number in site settings.</p>{whatsappHref ? <a className="premium-whatsapp-action" href={whatsappHref} target="_blank" rel="noreferrer">Open WhatsApp</a> : <p className="premium-aside-notice">The WhatsApp line has not yet been published. Use the enquiry form instead.</p>}<form className="premium-newsletter-form" onSubmit={onNewsletter}><p className="premium-eyebrow">Newsletter</p><label>Email updates<input name="email" required type="email" placeholder="you@example.com" /></label><button type="submit">Subscribe</button></form>{notice ? <p className="premium-success-note"><CheckCircle2 size={17} /> {notice}</p> : null}</aside></div></section></PublicShell>;
}
